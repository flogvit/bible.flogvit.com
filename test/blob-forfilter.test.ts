...
// `getPersonsByChapter` hentet 2029 rader og 7,7 MB tekst og parset hver
// eneste av dem, +33,3 MB flyktig rss. Tre søsterGettere gjorde det samme i
// det små. Til sammen ~43 MB per kapittelrender, på den mest besøkte flata vi
// har (1189 kapitler × 8 språk) — og det er dette #110 måler enden av:
// residenten klatrer 15 MB/t og flater aldri ut mens live-settet står stille.
//
// Vakta har fem halvdeler, og bare den siste er den vanlige:
//
...
//   SVARET        getteren gir ordrett det den ga før, for adresser valgt av
...
describe('SVARET — getteren gir ordrett det den ga før', () => {
  ...
  test('adressene velges av DATAENE, og de to veiene er enige', async () => {
    ...
    const rader = (await sql`
      SELECT content FROM persons WHERE language = 'nb' LIMIT 400
    `) as { content: string }[];
  ...
  test('en rad med NULL i kolonnen finnes fortsatt — fail-safe hele veien', async () => {
    ...
    const før = (await sql`SELECT ref_books AS v FROM persons WHERE id = ${rad!.id}`) as { v: string }[];
    try {
      await sql`UPDATE persons SET ref_books = NULL WHERE id = ${rad!.id}`;
      const funnet = await getPersonsByChapter(ref.bookId, ref.chapterId, 'nb');
      expect(funnet.map((p) => p.id)).toContain(person.id);
    } finally {
      await sql`UPDATE persons SET ref_books = ${før[0]!.v} WHERE id = ${rad!.id}`;
    }
  });
});
...
describe('FORMEN — hver getter som sveiper en hel blobb-tabell narrer først', () => {
  test('de fire getterne parser ikke en rad de ikke har forfiltrert', () => {
    // MINNET-halvdelen under måler `persons`, som er 33 av de 43 MB-ene. De
    ...
    const KRAV: Record<string, RegExp> = {
      getPersonsByChapter: /refBooksMayMatch\(/,
      getThemesByChapter: /bookMentionTest\(/,
      getStoriesByChapter: /bookMentionTest\(/,
      getNumberSymbolismByChapter: /bookMentionTest\(/,
    };
...
describe('DEPLOYEN — kolonnen synkes der prod faktisk får den', () => {
  ...
  test('en rad som har drevet fra innholdet rettes, og synken er idempotent', async () => {
    ...
    await sql`UPDATE persons SET ref_books = ',999,' WHERE id = ${rad!.id}`;
    const første = await syncPersonRefBooks(sql);
    expect(første.oppdatert).toBeGreaterThan(0);
    const [etter] = (await sql`SELECT ref_books AS v FROM persons WHERE id = ${rad!.id}`) as { v: string }[];
  ...
  test('ensureSchema() etterlater ingen rad som er uenig med sitt eget innhold', async () => {
    ...
    const uenige = (await sql`
      SELECT id, content, ref_books FROM persons WHERE ref_books IS NULL LIMIT 5
    `) as { id: number }[];
...
describe('MINNET — den nye veien koster en brøkdel av den gamle', () => {
  test('oppslaget vokser mindre enn halvparten av det gamle', async () => {
    ...
    // er en egenskap ved koden. Målt lokalt: 57–70 MB gammel mot 12–17 MB ny.

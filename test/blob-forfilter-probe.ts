...
// Fast streng, ett `?`-parameter — ordrett spørringen getteren hadde før #110.
const GAMMEL_SQL = 'SELECT content FROM persons WHERE language = ?';
...
async function gammelVei(bookId: number, chapter: number, lang: string): Promise<number> {
  ...
  for (const language of contentLanguageChain(lang)) {
    const rows = (await sql.unsafe(GAMMEL_SQL, [language])) as unknown as { content: string }[];

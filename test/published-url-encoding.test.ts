...
// Kapittelsida er den dyreste renderen vi har, og de fire halvdelene under
// spør om SAMME side. Hentet én gang per (språk, bok) er det 32 render for
// hele fila framfor ~100 — og alle halvdelene måler da nøyaktig samme svar.
//
// Statusen bæres med framfor å bli forutsatt: er sida ikke 200, er `canonical`
// borte, og en `!` ville gitt «undefined is not an object» der feilmeldingen
// skal si hvilken side som ikke svarte.
const pages = new Map<string, { status: number; html: string }>();
async function page(locale: Locale, slug: string) {
  const key = `${locale} ${slug}`;
  if (!pages.has(key)) {
    const res = await app.request(encodeURI(`/${locale}/${slug}/1`));
    pages.set(key, { status: res.status, html: await res.text() });
  }
  const got = pages.get(key)!;
  expect({ side: key, status: got.status }).toEqual({ side: key, status: 200 });
  return got.html;
}
...
describe('publiserte adresser (#80)', () => {
  ...
  test('ingen publisert adresse bærer et tegn utenfor ASCII', async () => {
    for (const book of NON_ASCII_BOOKS) {
      for (const locale of LOCALES) {
        ...
        const path = `/${locale}/${slug}/1`;
        const rå = publishedUrls(await page(locale, slug)).filter((u) => /[^\x20-\x7e]/.test(u));
  ...
  test('den kodede adressen dekoder tilbake til sidas egen sti', async () => {
    for (const book of NON_ASCII_BOOKS) {
      const slug = toUrlSlug(book.short_name);
      for (const locale of LOCALES) {
        const html = await page(locale, slug);
        expect({
          slug,
          locale,
  ...
  test('delekortet svarer 200 på adressen slik den er publisert', async () => {
    for (const book of NON_ASCII_BOOKS) {
      const url = ogImageOf(await page('en', toUrlSlug(book.short_name)))!;
